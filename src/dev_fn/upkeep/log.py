...
from .empty import keep_first_fn

try:
    from termcolor import colored
except ImportError:
    colored = keep_first_fn
...
def configure_mp_main(queue: mp.Queue):
    ...
    _log_listener = logging.handlers.QueueListener(
        queue, *_root_logger.handlers, respect_handler_level=True
    )
    _log_listener.start()
...
class ConsoleFormatter(Formatter):
    FORMATS = {
        logging.DEBUG: colored(
            " ".join([Formatter.time_str, Formatter.level_str, Formatter.src_str, ""]),
            "cyan",
            attrs=["dark"],
        )
