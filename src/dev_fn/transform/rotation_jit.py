# adapted from pytorch3d.transformation (f816568)
...
@torch.jit.script
def standardize_quat(quat: torch.Tensor) -> torch.Tensor:
    """
...
@torch.jit.script
def normalize_quat(quat: torch.Tensor, eps: Optional[float] = 1.1920928955078125e-07) -> torch.Tensor:
    """
    ...
    Returns:
        normalized quaternions in form (w,x,y,z),
        tensor of shape (..., 4).
    """
...
def quat_multiply(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    ...
    """
    ab = quat_raw_multiply(a, b)
    return standardize_quat(ab)
...
@torch.jit.script
def quat_apply(quat: torch.Tensor, point: torch.Tensor) -> torch.Tensor:
    """
    ...
    point_as_quat = torch.cat((real_parts, point), -1)
    out = quat_raw_multiply(
        quat_raw_multiply(quat, point_as_quat),
        quat_invert(quat),
    )
    return out[..., 1:]
...
def rotvec_to_rotmat(rotvec: torch.Tensor) -> torch.Tensor:
    ...
    """
    return quat_to_rotmat(rotvec_to_quat(rotvec))
...
def rotmat_to_rotvec(rotmat: torch.Tensor) -> torch.Tensor:
    ...
    Returns:
        Rotations given as a vector in axis angle form, as a tensor
            ...
            direction.
    """
    return quat_to_rotvec(rotmat_to_quat(rotmat))
...
@torch.jit.script
def euler_angle_to_rotmat(euler_angles: torch.Tensor, convention: str) -> torch.Tensor:
    """
...
@torch.jit.script
def euler_angle_to_quat(euler_angles: torch.Tensor, convention: str) -> torch.Tensor:
    """
    ...
    Returns:
        quaternions with real part first, as tensor of shape (..., 4).
    """
    return rotmat_to_quat(euler_angle_to_rotmat(euler_angles, convention))
...
@torch.jit.script
def rotmat_to_euler_angle(matrix: torch.Tensor, convention: str) -> torch.Tensor:
    """
    ...
    if tait_bryan:
        ...
        _v = torch.clamp(_v, -1.0 + eps, 1.0 - eps)
        central_angle = torch.asin(_v)
    else:
...
@torch.jit.script
def quat_to_euler_angle(quat: torch.Tensor, convention: str) -> torch.Tensor:
    """
